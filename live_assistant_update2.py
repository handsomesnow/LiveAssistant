...
class PomodoroTimer:
    ...
    def stop(self):
        ...
        self.total_seconds = 0
        self.paused_time = 0
        self.app.update_pomodoro_display(0)
    ...
    def is_playing():
        def callback(hwnd, handles):
            if win32gui.IsWindowVisible(hwnd):
                ...
                if "哔哩哔哩" in title and ("播放" in title or " - " in title):
                    handles.append(hwnd)
            return True
...
class LiveAssistant(tk.Tk):
    ...
    def init_ui(self):
        ...
        self.pomodoro_frame.pack(pady=(10, 20), padx=20, fill='x')
    ...
    def init_pomodoro_ui(self):
        ...
        # 控制按钮
        btn_frame = tk.Frame(self.pomodoro_frame, bg='#1A1A1A')
        btn_frame.pack(side='top', pady=5)

        self.start_btn = ttk.Button(btn_frame, text="开始", command=self.start_pomodoro)
        self.start_btn.pack(side='left', padx=5)

        self.pause_btn = ttk.Button(btn_frame, text="暂停", command=self.pause_pomodoro)
        self.pause_btn.pack(side='left', padx=5)

        self.stop_btn = ttk.Button(btn_frame, text="停止", command=self.stop_pomodoro)
    ...
    def check_bilibili(self):
        """检测哔哩哔哩播放状态"""
        if BilibiliMonitor.is_playing() and self.pomodoro.is_running:
            self.pause_pomodoro()
            self.label.config(text="检测到哔哩哔哩播放，番茄钟已暂停", fg='#FF0000')
        self.after(5000, self.check_bilibili)
